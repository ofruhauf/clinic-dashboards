 ...
 * sessions CRM export (sessions), or a full snapshot (any combination).
 * Whichever of `rows` / `registeredPatients` is present in `parsed` gets
 * merged into the matching part of the existing dataset.
 *
 * `sessions` is different: it's not merged/deduped row-by-row like the
 * other two, it REPLACES whatever sessions data already existed. A sessions
 * export is a snapshot of the CRM's complete session list as of the moment
 * it was pulled — the previous upload's rows are simply superseded by a
 * fresh pull, so there's nothing meaningful to merge them against.
 */
export function mergeIntoDataset(
  ...
  const sessions = parsed.sessions ?? existing?.sessions ?? [];
